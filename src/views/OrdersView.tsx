                ...
                <div className="whitespace-nowrap overflow-hidden">
                    ...
                    <div className="flex items-center gap-2 mt-1 md:mt-2">
                             {[
                                ...
                                className={`flex items-center gap-1.5 px-3 py-1 rounded-full text-[9px] md:text-[10px] font-bold transition-all border-2 ${selectedVertical === v.id ? `bg-${v.color}-500 border-${v.color}-500 text-white` : 'bg-white border-gray-100 text-gray-400'}`}
                            >
            ...
            <div className="bg-white border border-gray-100 rounded-3xl shadow-sm flex-grow overflow-hidden flex flex-col">
                <div className="p-3 md:p-4 border-b border-gray-50 flex items-center gap-2 md:gap-4 overflow-x-auto no-scrollbar">
                    ...
                    <button
                        ...
                        className={`px-3 py-1.5 md:px-4 md:py-2 rounded-lg md:rounded-xl text-[10px] md:text-xs font-bold transition-all whitespace-nowrap ${filterStatus === 'PENDING' ? 'bg-amber-500 text-white shadow-lg shadow-amber-100' : 'text-gray-400 hover:text-gray-900'}`}
                    >
                        Commandées ({localOrders.filter(o => o.status === 'PENDING').length})
                    </button>
                    <button
                        onClick={() => setFilterStatus('READY')}
                        className={`px-3 py-1.5 md:px-4 md:py-2 rounded-lg md:rounded-xl text-[10px] md:text-xs font-bold transition-all whitespace-nowrap ${filterStatus === 'READY' ? 'bg-blue-500 text-white shadow-lg shadow-blue-100' : 'text-gray-400 hover:text-gray-900'}`}
                    >
                        En cours ({localOrders.filter(o => o.status === 'READY').length})
                    </button>
                    <button
                        onClick={() => setFilterStatus('COMPLETED')}
                        className={`px-3 py-1.5 md:px-4 md:py-2 rounded-lg md:rounded-xl text-[10px] md:text-xs font-bold transition-all whitespace-nowrap ${filterStatus === 'COMPLETED' ? 'bg-green-500 text-white shadow-lg shadow-green-100' : 'text-gray-400 hover:text-gray-900'}`}
                    >
                    ...
                    {selectedOrderIds.length > 0 && (
                        <div className="flex items-center gap-1.5 md:gap-2">
                            <button 
                                ...
                                className="px-2.5 py-1.5 bg-amber-500 hover:bg-amber-600 text-white rounded-lg text-[10px] md:text-xs font-bold transition-colors shadow-sm whitespace-nowrap"
                            >
                                Commandées
                            </button>
                            <button 
                                onClick={() => handleBulkUpdateStatus('READY')}
                                className="px-2.5 py-1.5 bg-blue-500 hover:bg-blue-600 text-white rounded-lg text-[10px] md:text-xs font-bold transition-colors shadow-sm whitespace-nowrap"
                            >
                                En cours
                            </button>
                            <button 
                                onClick={() => handleBulkUpdateStatus('COMPLETED')}
                                className="px-2.5 py-1.5 bg-green-500 hover:bg-green-600 text-white rounded-lg text-[10px] md:text-xs font-bold transition-colors shadow-sm whitespace-nowrap"
                            >
                                Livrées
                            </button>
                            <button 
                                onClick={handleBulkDelete}
                                className="px-2.5 py-1.5 bg-red-50 text-red-600 border border-red-100 hover:bg-red-100 rounded-lg text-[10px] md:text-xs font-bold transition-colors shadow-sm whitespace-nowrap flex items-center gap-1"
                            >
                ...
                <div className="overflow-y-auto flex-grow custom-scrollbar">
                    {filteredOrders.length > 0 ? (
                                    ...
                                    >
                                        ...
                                        <div className="flex-grow min-w-0 flex items-center justify-between gap-2 overflow-hidden">
                                            <div className="flex flex-col gap-1.5 min-w-0">
                                                <div className="flex items-center gap-2">
                                                    <span className="text-[11px] md:text-sm font-bold text-gray-900 whitespace-nowrap">#{order.id.slice(-6).toUpperCase()}</span>
                                                    <span className="text-[9px] md:text-xs font-semibold text-gray-500 truncate max-w-[120px] md:max-w-none whitespace-nowrap">
                                                        {order.customer?.name || 'Passage'}
                                                    </span>
                                                    <span className={`text-[7px] md:text-[8px] font-bold px-1.5 py-0.5 rounded whitespace-nowrap tracking-wider ${order.type === 'PICKUP' ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-600'}`}>
                                            ...
                                            <div className="flex items-center gap-1.5 md:gap-2 flex-shrink-0">
                                                ...
                                                <div className="hidden md:block text-right">
                                                    <div className={`text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-tighter ${getStatusColor(order.status || 'READY')}`}>
                                                        {getStatusLabel(order.status || 'READY')}
                                                    </div>
                        ...
                        <div className="h-full flex flex-col items-center justify-center p-20 text-center gap-4">
                            ...
                            <div>
                                <p className="text-lg font-bold text-gray-900 leading-tight">Aucune commande trouvée</p>
                    ...
                    <div className="bg-white h-[85vh] md:h-auto md:max-h-[85vh] w-full md:max-w-xl md:rounded-3xl shadow-2xl flex flex-col animate-slide-up md:animate-in md:zoom-in-95 md:duration-500 overflow-hidden rounded-t-3xl md:rounded-3xl">
                        <div className="p-4 md:p-6 border-b border-gray-100 flex items-center justify-between sticky top-0 bg-white z-10">
                            ...
                            <div className="flex items-center gap-3 md:gap-4 mt-2 md:mt-0">
                                ...
                                <div>
                                    <h2 className="text-lg md:text-xl font-bold text-gray-900 tracking-tight whitespace-nowrap">Commande #{selectedOrder.id.slice(-6).toUpperCase()}</h2>
                                    <div className="flex items-center flex-wrap gap-1.5 md:gap-2 mt-1">
                                        <div className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[9px] md:text-[10px] font-bold uppercase tracking-wider whitespace-nowrap ${getStatusColor(selectedOrder.status || 'READY')}`}>
                                            {getStatusLabel(selectedOrder.status || 'READY')}
                                        </div>
                                        <div className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded-md text-[9px] md:text-[10px] font-bold uppercase tracking-wider whitespace-nowrap ${selectedOrder.type === 'PICKUP' ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-600'}`}>
                                            {selectedOrder.type === 'PICKUP' ? 'Marketplace' : 'Boutique'}
                                        </div>
                                        <div className="flex items-center gap-1 text-[10px] md:text-xs font-semibold text-gray-500 whitespace-nowrap">
                            ...
                            <div className="space-y-6 md:space-y-8">
                                ...
                                <div className="space-y-3 md:space-y-4">
                                    ...
                                    <div className="flex items-center gap-3 md:gap-4 p-3 md:p-4 bg-gray-50 rounded-2xl border border-gray-100">
                                        ...
                                        <div className="min-w-0">
                                            <div className="font-bold text-gray-900 truncate">{selectedOrder.customer?.name || 'Client de passage'}</div>
                                ...
                                <div className="space-y-3 md:space-y-4">
                                    <div className="flex items-center justify-between">
                                        <h3 className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Articles ({loadingOrderItems ? '...' : selectedOrderItems.length})</h3>
                                    </div>
                                    {loadingOrderItems ? (
...
{selectedOrderItems.map((item, idx) => {
                                            ...
                                            <div key={idx} className="flex items-center gap-3 md:gap-4 p-2 md:p-3 border border-gray-50 rounded-xl md:rounded-2xl hover:bg-gray-50/50 transition-colors">
                                                ...
                                                <div className="text-right">
                                                    <div className="text-xs md:text-sm font-bold text-gray-900">{formatCurrency(lineTotal)}</div>
                                                    {isWholesale && (
                                                        <div className="text-[9px] md:text-[10px] font-semibold text-gray-400 line-through">{formatCurrency(baseUnit * unitQty)}</div>
                                                    )}
                                                </div>
                                            </div>
                                            );
                                ...
                                {/* Payment Info */}
                                <div className="space-y-3 md:space-y-4">
                                    <h3 className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Paiement</h3>
                                    <div className="bg-gray-50 p-4 md:p-6 rounded-2xl border border-gray-100 space-y-3 md:space-y-4">
                                        ...
                                        { (selectedOrder.discountAmount || 0) > 0 && (
                                            <div className="flex justify-between items-center text-xs md:text-sm text-green-600 bg-green-50/50 p-2 rounded-lg">
                                                <span className="font-semibold text-[10px] uppercase">Remise appliquée</span>
                                                <span className="font-bold">-{formatCurrency(selectedOrder.discountAmount || 0)}</span>
                                            </div>
                                        )}
                                        <div className="flex items-center gap-1.5 mt-3 md:mt-4 text-[9px] md:text-[10px] font-bold text-gray-400 uppercase tracking-wider">
                        ...
                        <div className="p-4 md:p-6 bg-white border-t border-gray-100 grid grid-cols-2 gap-3 md:gap-4">
                            <button
                                ...
                                className="py-3 md:py-4 px-4 md:px-6 bg-white border border-gray-200 rounded-xl md:rounded-2xl font-bold text-gray-600 hover:bg-gray-50 transition-all flex items-center justify-center gap-2 text-xs md:text-sm shadow-sm whitespace-nowrap"
                            >
                            ...
                            <button
                                ...
                                className={`py-3 md:py-4 px-4 md:px-6 flex items-center justify-center gap-2 text-xs md:text-sm font-bold rounded-xl md:rounded-2xl shadow-xl shadow-orange-100 transition-all whitespace-nowrap disabled:opacity-50 disabled:cursor-not-allowed ${
