        ...
        <div className="flex flex-col md:flex-row md:items-center gap-6">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex flex-wrap items-center gap-3">
              <h1 className="text-2xl md:text-3xl font-bold text-gray-900 uppercase tracking-tighter">#{order.id.split('-')[0]?.toUpperCase()}</h1>
              {(order.status && statusLabels[order.status]) ? (
                <span className={`px-3 py-1 rounded-lg text-[9px] font-bold uppercase border ${statusLabels[order.status].color}`}>
                  {statusLabels[order.status].label}
                </span>
              ) : (
                <span className="px-3 py-1 rounded-lg text-[9px] font-bold uppercase border bg-gray-50 text-gray-500 border-gray-100">{order.status || '—'}</span>
              )}
            </div>
            <div className="flex flex-wrap items-center gap-4 mt-3 text-xs font-semibold text-gray-400">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          {store ? (
            <Link href={`/pam/stores/${store.id}`} className="flex items-center gap-4 p-4 bg-gray-50/50 rounded-2xl border border-gray-100 hover:bg-orange-50/30 transition-all">
              ...
              <div>
                ...
                <p className="text-[10px] font-semibold text-gray-400 font-mono">/{store.slug}</p>
              </div>
            </Link>
          ) : (
            <p className="text-sm text-gray-400 font-semibold">Boutique inconnue</p>
          )}
        </div>
        ...
        <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-6">
          ...
          <div className="p-4 bg-gray-50/50 rounded-2xl border border-gray-100 space-y-2">
            ...
            <div className="flex items-center justify-between text-xs">
              <span className="text-gray-400 font-semibold">Sous-total</span>
              <span className="font-bold text-gray-900">{formatCurrency(parseFloat(order.subtotal ?? '') || 0)}</span>
            </div>
            {order.discountAmount && parseFloat(order.discountAmount) > 0 && (
              <div className="flex items-center justify-between text-xs">
                <span className="text-gray-400 font-semibold">Remise {(order.promoCode ? `(${order.promoCode})` : '')}</span>
                <span className="font-bold text-red-500">-{formatCurrency(parseFloat(order.discountAmount))}</span>
              </div>
            )}
          ...
          {items.map((item) => (
            <div key={item.id} className="flex items-center justify-between p-4 bg-gray-50/50 rounded-2xl border border-gray-100">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-xs font-bold text-gray-900">Produit #{item.productId?.split('-')[0]?.toUpperCase() || '—'}</p>
                  <p className="text-[10px] font-semibold text-gray-400">{item.quantity} × {formatCurrency(parseFloat(item.unitPrice ?? '') || 0)}</p>
                </div>
              </div>
              <p className="text-xs font-bold text-gray-900">{formatCurrency(parseFloat(item.total ?? '') || 0)}</p>
            </div>
          ))}
        </div>
      </div>
