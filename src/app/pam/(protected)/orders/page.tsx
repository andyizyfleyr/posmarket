...
export default function AdminOrdersPage() {
  ...
  const statusBadge = (status?: string | null) => {
    switch (status) {
      case 'PENDING': return <span className="px-2 py-0.5 bg-yellow-50 text-yellow-600 text-[8px] font-bold rounded-md border border-yellow-100 uppercase">En attente</span>;
      case 'READY': return <span className="px-2 py-0.5 bg-blue-50 text-blue-600 text-[8px] font-bold rounded-md border border-blue-100 uppercase">Prête</span>;
      case 'COMPLETED': return <span className="px-2 py-0.5 bg-emerald-50 text-emerald-600 text-[8px] font-bold rounded-md border border-emerald-100 uppercase">Validée</span>;
      default: return <span className="px-2 py-0.5 bg-gray-50 text-gray-500 text-[8px] font-bold rounded-md border border-gray-100 uppercase">{status || '—'}</span>;
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl md:text-3xl font-bold text-gray-900 uppercase tracking-tighter">Transactions</h1>
        <p className="text-xs text-gray-400 font-semibold uppercase tracking-widest mt-1">Toutes les commandes de la plateforme ({orders.length})</p>
      </div>
      ...
      <div className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
        {filtered.length === 0 && (
          <div className="p-12 text-center text-gray-400 text-sm font-semibold">Aucune commande trouvée</div>
        )}
            ...
            return (
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-xs font-bold text-gray-900">#{o.id.split('-')[0]?.toUpperCase()}</p>
                    ...
                    {o.buyer_email && (
                      <p className="text-[9px] font-semibold text-gray-300 lowercase mt-0.5">{o.buyer_email}</p>
                    )}
                  </div>
                </div>
