  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-gray-900 uppercase tracking-tighter">Surveillance du Réseau</h1>
          <p className="text-xs text-gray-400 font-semibold uppercase tracking-widest mt-1">Vue d&apos;ensemble globale de la plateforme</p>
        </div>
      </div>
        ...
        <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-6 relative overflow-hidden h-fit">
          ...
          {stats.pendingStores > 0 && (
            <Link href="/pam/stores" className="flex items-center gap-3 p-4 mb-4 bg-yellow-50 rounded-2xl border border-yellow-100 hover:bg-yellow-100/60 transition-all">
              ...
              <div>
                <p className="text-xs font-bold text-yellow-700">{stats.pendingStores} boutique(s) en attente de validation</p>
                <p className="text-[10px] font-semibold text-yellow-500 uppercase tracking-widest mt-0.5">Action requise</p>
              </div>
            </Link>
          )}
            ...
            {[
              ...
              <div key={idx} className="p-3 bg-gray-50 rounded-2xl border border-gray-100">
                <div className="flex justify-between items-center mb-1">
                  <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">{item.l}</span>
                  <div className={`w-2 h-2 rounded-full bg-${item.s}-500`} />
                </div>
                <div className="text-xs font-bold text-gray-700">{item.v}</div>
              </div>
            ))}
          </div>
        </div>
